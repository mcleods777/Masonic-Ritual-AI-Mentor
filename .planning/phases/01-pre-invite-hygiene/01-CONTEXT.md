...
- **D-12:** Assertion matrix: the test runs the compiled matcher against **a small representative path set** — `/foo.mram`, `/deeply/nested/path/ritual.mram`, `/ea-degree.mram`, `/FOO.MRAM` (uppercase sanity check). Each must not match. A single-path test would miss nested / hyphen / case regressions.
...
- **D-16:** Run `npx @ai-sdk/codemod upgrade v6` per the Vercel upgrade docs. Verification: `npm run build` + `npm test` + a manual smoke of `/api/rehearsal-feedback` (the one route that actually imports `ai`). If the codemod produces a broken build, **review the diff and fix by hand in the same commit** — do not revert and defer. Codemod edge cases are mechanical; Phase 5 will re-do this region anyway, but we want v6 idioms in the tree now so Phase 5 isn't also paying migration tax.
- **D-17:** The codemod runs **after** HYGIENE-01 dead-package removal, to reduce the surface area the codemod walks.
