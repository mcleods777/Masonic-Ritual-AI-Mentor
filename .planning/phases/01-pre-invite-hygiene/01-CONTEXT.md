...
- **D-11:** Test location: **`src/__tests__/middleware.test.ts`**. Updated 2026-04-20 post-pattern-mapping: the repo's actual test convention is `__tests__/` subdirectories (see `src/lib/__tests__/rehearsal-decision.test.ts`), not co-location. The original wording "co-located matches the vitest convention of tests next to source" was factually wrong — no co-located tests exist in this repo. The new test file lives at `src/__tests__/middleware.test.ts` (directly under src/ since middleware.ts is at src/middleware.ts, not inside src/lib/). Vitest's default glob `src/**/*.test.{ts,tsx}` finds it regardless.
